...
def index(request):
    ...
    return render(request,'aby/index.html',{'image':obj})

def detail_view(request,id,slug):
    obj=get_object_or_404(Image,id=id,slug=slug)
    return render(request,'aby/detail.html',{'image':obj})
...
def search(request):
    ...
    return render(request,'aby/search.html',{'query':query,'result':result})